...
const getComponent = async (props) => {
	try {
		return await (
			await getComponentOutput('./packages/typetura/components/SectionSubHeadline.astro', { ...props })
		).html;
	} catch (error) {
		return error;
	}
...
describe('Test : <SectionSubHeadline/>', () => {
	describe('Test range of <SectionSubHeadline/> Components', () => {
		let component;
	...
	describe('👓 <SectionSubHeadline level="h3" key="special" base={42} scale={0.5} ease=\'ease-in\' id="test"/>', () => {
		let component;
		before(async () => {
			component = await getComponent({ level: 'h3', key: 'special', ease: 'ease-in', base: 42, scale: 0.5, id: 'test', class: 'flex red' });
		});
		it('Render: `<style>.section-subheadline-**UUID** {--tt-key:special;--tt-ease:ease-in;--tt-base:42;--tt-scale:0.5;}</style><h4 class="primary-headline-**UUID** flex red"/>', () => {
			expect(component)
				.to.include('style')
				.and.to.include('section-subheadline')
