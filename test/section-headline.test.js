...
const getComponent = async (props) => {
	try {
		return await (
			await getComponentOutput('./packages/typetura/components/SectionHeadline.astro', { ...props })
		).html;
	} catch (error) {
		return error;
	}
...
describe('Test : <SectionHeadline/>', () => {
	describe('Test range of <SectionHeadline/> Components', () => {
		let component;
	...
	describe('👓 <SectionHeadline key="special" base={42} scale={0.5} ease=\'ease-in\' id="test"/>', () => {
		let component;
		before(async () => {
			component = await getComponent({ level: 'h3', key: 'special', ease: 'ease-in', base: 42, scale: 0.5, id: 'test', class: 'flex red' });
		});
		it('Render: `<style>.section-headline-**UUID** {--tt-key:special;--tt-ease:ease-in;--tt-base:42;--tt-scale:0.5;}</style><h2 class="primary-headline-**UUID** flex red"/>', () => {
			expect(component)
				.to.include('style')
				.and.to.include('section-headline')
